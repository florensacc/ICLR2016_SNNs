...
In Setion~\ref{section:method:inforeg}, we describe an information-theoretic regularizer that further improves the span of skills learned by SNNs. Finally, in Section~\ref{section:method:highlevel}, we describe the architecture of high-level policies over the learned skills, and the training procedure for the downstream tasks with sparse rewards.%training of high-level policies over the learned skills in the downstream tasks.
...
\begin{figure}[h]
	...
	\subfigure[Combined visitation of the independent policies]{
		\centering
	...
	\subfigure[SNN without bilinear integration and increasing $\alpha_H=0,0.001,0.01,0.1$]{
		\centering
	...
	\subfigure[SNN with bilinear integration and increasing $\alpha_H=0,0.001,0.01,0.1$]{
		\centering
...
\begin{figure}[h!]
	...
	\subfigure[Gaussian noise with covariance $\Sigma=I$]{
		...
		\includegraphics[trim={2cm 0 1cm 1.5cm}, clip, width = 0.22\textwidth]{Figures/visit-trpo1M.png}
	}
	...
	\subfigure[Hierarchy with Bilinear-SNN, $\alpha_H=0.01$]{
		...
		\includegraphics[trim={2cm 0 1cm 1.5cm}, clip, width = 0.22\textwidth]{Figures/visit-snnHB-1.png}
	}
	\caption{Visitation plots for different randomly initialized architectures (100 rollouts, 10k path length)}
