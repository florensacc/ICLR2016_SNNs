...
\section{Snake}
\label{sec:snake}
In this section we study how our method scales to a more complex robot. We repeat some experiments from Section 7 with a 5-link robot, named Snake (compared to Swimmer the action dimension doubles and the state-space increases by 50\%). We then do some further analysis on the relevance of the switch time $\mathcal{T}$ and end up by reporting the performance variation among different pretrained SNNs on the hierarchical tasks.
